...
生成AI利用時の情報漏洩・プロンプトインジェクション・過大コンテキスト投入を防ぐための、ローカル安全フィルタCLI。

具体的には、以下のコマンドをAIエージェント/Skillから使わせて、巨大ログや危険情報を直接AIに渡さないようにする。
...
プロンプトインジェクション防止の要。生ログを直接AIに読ませない設計の中核となる仕組み。
